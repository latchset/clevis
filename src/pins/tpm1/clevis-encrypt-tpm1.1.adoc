...
IMPORTANT: If you changed the storage root key (SRK) to a _well-known key_, do
not forget to call `update-initramfs -u` (on Debian-like system) or `dracut -f`
(on Fedora-like system) to reinstall the necessary boot files, because the
